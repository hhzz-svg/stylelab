...
// minChapters 少于此章数只返回章数与 SampleTooSmall，频率没有意义。
const minChapters = 5

// phraseWindow 动态短语挖掘只看最近 N 章。
...
// Compute 计算全书风格统计；章数不足时返回 SampleTooSmall。
func Compute(in Input) *Stats {
	n := len(in.Chapters)
	if n < minChapters {
		return &Stats{Chapters: n, SampleTooSmall: true}
	}
